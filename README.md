...
3. 表单已预填代码上下文，补全 **标题 / 问题 / 根因 / 方案 / 收获 / 标签**。
   - 「根因 / 收获」最有思考深度，建议自己写；AI 起草的问题/方案可先让 WorkBuddy Skill 生成再粘贴。
...
3. 攒几条后，在 WorkBuddy 对话里说「**起草待办**」，Skill 批量读这些 pending-ai 条目、依 diff 起草 problem/solution 并写回（status 改 `draft`）。
4. 你补 `rootCause` / `lesson` / `tags` 后，改 `done` 即成为正式面试弹药。
